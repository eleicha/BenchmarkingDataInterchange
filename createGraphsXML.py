...
def draw_scatter_message_length(x1, x2, x3, x4, y, name, title, x_label, y_label):
	...
	print(len(x_protobuf))
	print(len(y_protobuf))
	...
	for path in paths:
		...
		for line in Lines:
			if line.split(':')[0] == 'CPU_UTIL':
				...
				if int(identify_format) == 0:
					message_length_proto.append(float(line.split(':')[1]))
				elif int(identify_format) == 1:
					message_length_capnp.append(float(line.split(':')[1]))
				elif int(identify_format) == 2:
					message_length_avro.append(float(line.split(':')[1]))
				elif int(identify_format) == 3:
					message_length_xml.append(float(line.split(':')[1]))
